...
class ik_solver:
    ...
    def movel2(self, matrix: np.ndarray):
        """
        4x4 NumPy 행렬을 받아 이동 명령을 수행합니다.
        matrix: 4x4 numpy array (float64)
        """
        if not self.is_initialized:
            return None

        # 1. NumPy 행렬의 형태가 4x4인지 확인
        if matrix.shape != (4, 4):
            raise ValueError("Input matrix must be a 4x4 numpy array")

        # 2. ik_solver_py.Transform 객체 생성 (C++ 바인딩에서 Eigen::Matrix4d 생성자 호출)
        # pybind11이 numpy를 Eigen::Ref<const Eigen::Matrix4d>로 변환해줍니다.
