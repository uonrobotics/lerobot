...
@PreTrainedConfig.register_subclass("groot_depth")
@dataclass
class GrootDepthConfig(PreTrainedConfig):
    """Configuration for GrootDepth policy wrapper."""
    ...
    def validate_features(self) -> None:
        """Validate and set up input/output features for GrootDepth."""
        image_features = [key for key, feat in self.input_features.items() if feat.type == FeatureType.VISUAL]
        if not image_features:
            raise ValueError(
                "GrootDepth policy requires at least one visual input feature. "
