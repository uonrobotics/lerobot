...
- Download and load the pretrained GR00T model via GR00TN15Depth.from_pretrained
...
from lerobot.policies.groot_depth.groot_n1_depth import GR00TN15Depth
from lerobot.policies.pretrained import PreTrainedPolicy
from lerobot.utils.constants import ACTION, OBS_IMAGES

T = TypeVar("T", bound="GrootDepthPolicy")

class GrootDepthPolicy(PreTrainedPolicy):
    """Wrapper around external GrootDepth model for LeRobot integration."""

    name = "groot_depth"
    config_class = GrootDepthConfig
    def __init__(self, config: GrootDepthConfig, **kwargs):
        """Initialize GrootDepth policy wrapper."""
    ...
    def _create_groot_model(self):
        ...
        1) Download and load pretrained model via GR00TN15Depth.from_pretrained
        2) Align action horizon with data_config if provided
        """
        # Handle Flash Attention compatibility issues
        self._handle_flash_attention_compatibility()

        model = GR00TN15Depth.from_pretrained(
    ...
    def from_pretrained(
        ...
        *,
        config: GrootDepthConfig | None = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            config: Optional GrootDepthConfig. If None, loads from checkpoint or creates default
        ...
        Returns:
            Initialized GrootDepthPolicy instance with loaded model
        """
        ...
        if config is None:
            ...
            config = GrootDepthConfig(base_model_path=str(pretrained_name_or_path))
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        allowed_base = {"state", "state_mask", "action", "action_mask", "embodiment_id", "wrist_depth"}
        groot_inputs = {
            k: v
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        allowed_base = {"state", "state_mask", "embodiment_id", "wrist_depth"}
        groot_inputs = {
            k: v
            for k, v in batch.items()
            if (k in allowed_base or k.startswith("eagle_")) and not (k.startswith("next.") or k == "info")
