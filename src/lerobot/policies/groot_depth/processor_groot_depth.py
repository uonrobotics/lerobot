...
def make_groot_depth_pre_post_processors(
    config: GrootDepthConfig, dataset_stats: dict[str, dict[str, torch.Tensor]] | None = None
) -> tuple[
    PolicyProcessorPipeline[dict[str, Any], dict[str, Any]],
    PolicyProcessorPipeline[PolicyAction, PolicyAction],
]:
    """Create preprocessor and postprocessor for GrootDepth policy.
    ...
    Args:
        config: GrootDepth configuration containing data_config, embodiment_tag, etc.
    ...
    output_steps: list[ProcessorStep] = [
        GrootDepthActionUnpackUnnormalizeStep(
...
@dataclass
@ProcessorStepRegistry.register(name="groot_depth_pack_inputs_v3")
class GrootDepthPackInputsStep(ProcessorStep):
    state_horizon: int = 1
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        # 1-2) Depth : 압축된 8-bit grayscale depth video
        depth_key = "observation.depth.cam_wrist"
        if depth_key in obs:
            depth = obs[depth_key]

            # Expect depth as (B, C, H, W) after hf_transform_to_torch
            ...
            # Current dataset stores depth as 3-channel grayscale video.
            # Since all channels are identical, keep only one channel.
            if depth.shape[1] == 3:
                depth = depth[:, :1, :, :]
            elif depth.shape[1] != 1:
                raise ValueError(f"depth channel must be 1 or 3, got {depth.shape[1]}")
            ...
            obs["wrist_depth"] = depth.contiguous()
...
@dataclass
@ProcessorStepRegistry.register(name="groot_depth_eagle_encode_v3")
class GrootDepthEagleEncodeStep(ProcessorStep):
...
@dataclass
@ProcessorStepRegistry.register(name="groot_depth_eagle_collate_v3")
class GrootDepthEagleCollateStep(ProcessorStep):
    tokenizer_assets_repo: str = DEFAULT_TOKENIZER_ASSETS_REPO
    _proc: ProcessorMixin | None = field(default=None, init=False, repr=False)
...
@dataclass
@ProcessorStepRegistry.register(name="groot_depth_action_unpack_unnormalize_v1")
class GrootDepthActionUnpackUnnormalizeStep(ProcessorStep):
