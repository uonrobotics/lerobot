...
#include "urdf_parser/urdf_parser.h" // urdfdom lib
using namespace std;
...
IkSolver::IkSolver(const std::string &urdf_path) : urdf_path(urdf_path)
{
    ...
    // Workspace Limit 설정
    ws_limit_min << -1.3, -1.3, 0.0; // [m]
    ws_limit_max << 1.3, 1.3, 1.0;   // [m]
    ...
    limit_mode = LimitMode::STRICT_MODE; // 경로 정확도 우선
...
bool IkSolver::init()
{
    ...
    // flange
    auto k = Kinematics::build_from_urdf(urdf, "flange");
    vec3 pos = k.get_origin_tf().translation();
    quat rot = k.get_origin_tf().quaternion();
    flange_offset = Transform(rot, pos);
...
void IkSolver::set_safety_scale(double scale)
{
    safety_factor = scale;
}

void IkSolver::set_joint_vlimit(int index, double max)
{
    ki[index].set_joint_vlimit(max);
}

void IkSolver::set_end_effector_tf(const Transform& tf)
{
    end_effector = tf;
    end_effector_inverse = tf.inverse();
}

Transform IkSolver::get_tcp_tf() const
{
    return fk_cache[5] * flange_offset * end_effector;
}

Transform IkSolver::get_tf(int index) const
{
    if (index < 0 || index >= 6) return Transform();
    return fk_cache[index];
}

bool IkSolver::has_end_effector() const
{
    return !end_effector.matrix().isIdentity();
}

void IkSolver::remove_end_effector()
{
    end_effector = Transform();
    end_effector_inverse = Transform();
}

void IkSolver::movej(const double(&q)[6])
{
...
{
    const vec<6> tar_q = {q0, q1, q2, q3, q4, q5};
    ...
    // 키네마틱스 업데이트
    ki[0].set_joint_value(q0);
    ki[1].set_joint_value(q1);
    ki[2].set_joint_value(q2);
    ki[3].set_joint_value(q3);
    ki[4].set_joint_value(q4);
    ki[5].set_joint_value(q5);
...
double IkSolver::dt()
{
    ...
    const std::chrono::duration<double> diff = now - lastTime_;
    lastTime_ = now;
...
vec<6> IkSolver::ik(const Transform& tar_tf, double dt)
{
    ...
    if (limit_mode == STRICT_MODE) {
        // [STRICT_PATH 모드]: Cartesian Time-Scaling 적용 (TCP 경로 철저히 유지)
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            const double limit = ki[i].get_joint_limits().velocity / safety_factor;
...
vec<6> IkSolver::ik_core(const Transform& tar_flange, double dt)
{
    ...
    while (i --> 0) {
        ...
        if (p_error < POS_ERROR_THRESHOLD && r_error < ROT_ERROR_THRESHOLD)
            break;
...
IkSolver::QPProblem IkSolver::setup_QP(const mat<6, 6>& J, const vec<6>& dx, const vec<6>& cur_q, double dt) const
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        double max_vel = ki[i].get_joint_limits().velocity / safety_factor;
...
vec<6> IkSolver::limit_workspace(const vec3& cur_pos, const vec<6>& dx) const
{
    ...
    for (int i = 0; i < 3; ++i) { // 위치 x, y, z
        // --- 최소값(Min) 체크 및 보정 ---
        const double dist_min = cur_pos(i) - ws_limit_min(i);
        if (dist_min < 0.0) {
            // 경계 침범: 강제 복원 속도 계산
            const double recovery_vel = -dist_min * WS_P_GAIN;
            if (limited_dx(i) < recovery_vel) limited_dx(i) = recovery_vel;
        }
        else if (dist_min < WS_LIMIT_MARGIN) {
            // 경고 마진: 밖으로 나가는 속도 감쇄
            if (limited_dx(i) < 0) {
                const double ratio = dist_min / ((WS_LIMIT_MARGIN > 1e-6) ? WS_LIMIT_MARGIN : 1.0);
                limited_dx(i) *= ratio;
            }
        }

        // --- 최대값(Max) 체크 및 보정 ---
        const double dist_max = ws_limit_max(i) - cur_pos(i);
        if (dist_max < 0.0) {
            // 경계 침범: 강제 복원 속도 계산
            const double recovery_vel = dist_max * WS_P_GAIN;
            if (limited_dx(i) > recovery_vel) limited_dx(i) = recovery_vel;
        }
        else if (dist_max < WS_LIMIT_MARGIN) {
            // 경고 마진: 밖으로 나가는 속도 감쇄
            if (limited_dx(i) > 0) {
                const double ratio = dist_max / ((WS_LIMIT_MARGIN > 1e-6) ? WS_LIMIT_MARGIN : 1.0);
                limited_dx(i) *= ratio;
            }
        }
    }
